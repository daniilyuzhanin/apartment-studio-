...
export function createApartment(container,onRoom){
 ...
 let root,roomFloors=[],labels=[],state,selected='living',palette,activeView='iso';
 ...
 function bed(layout){const g=group(1.38,layout.config.bedTail?2.70:3.13,Math.PI/2);round(1.67,.28,2.12,0,.13,0,palette.wood,.06,g);round(1.6,.23,2.03,0,.41,0,'#eee5d5',.1,g);round(1.63,.08,1.28,0,.64,.34,palette.accent,.06,g);round(1.85,.90,.12,0,.18,-1.09,palette.fabric,.07,g);for(const xx of [-.43,.43])round(.64,.13,.43,xx,.66,-.72,'#f6eee0',.09,g);for(const xx of [-1.03,1.03]){cyl(.24,.43,xx,0,-.85,palette.wood,g);cyl(.07,.28,xx,.43,-.85,'#716f51',g);sphere(.13,.10,.13,xx,.73,-.85,'#f0e5c8',g);}for(let i=0;i<(layout.config.bedTail?3:4);i++){const x=(layout.config.bedTail?1.05:.52)+i*.70;box(.69,2.25,.56,x,0,4.98,palette.wall);box(.012,.28,.018,x+.29,1.06,4.69,palette.wood);}rug(1.55,layout.config.bedTail?2.65:3.1,2.6,layout.config.bedTail?2.05:2.8);}
 ...
 function kitchen(){
  for(let i=0;i<6;i++){const x=10.65+i*.6;const tall=i===5;const h=tall?2.3:.86;box(.584,h,.62,x,.06,3.40,palette.kitchen);box(.57,.035,.017,x,h*.67,3.077,palette.wood);if(tall)box(.57,.028,.012,x,1.38,3.075,'#525d4d');}
  box(3.44,.055,.67,12.22,.94,3.4,'#e4ddcb');box(2.8,.35,.035,12.1,1.0,3.74,'#d4d3b9');
 ...
 function bathroom(architecture){
  ...
  for(const f of architecture.showerFixtures){
   if(f.id==='cabinet'){
    for(let i=0;i<3;i++){box(f.w/3-.012,f.h,f.d,f.x-f.w/3+i*f.w/3,.04,f.z,palette.kitchen);box(.025,.26,.026,f.x-f.w/3+i*f.w/3+.18,1.02,f.z+f.d/2+.015,palette.wood);}
   }else if(f.id==='vanity'){
    box(f.w,f.h,f.d,f.x,.04,f.z,palette.wood);round(f.w,.055,f.d,f.x,f.h+.04,f.z,'#f2f0e7',.025);round(f.w-.15,.02,f.d-.13,f.x,f.h+.10,f.z,'#b3c3bc',.08);
    box(f.w,.8,.025,f.x,1.10,6.155,material('#a5bcb6',{metalness:.7,roughness:.14}));cyl(.016,.20,f.x,f.h+.10,6.10,'#68766e');
   }else if(f.id==='shower-tray'){
    box(f.w,f.h,f.d,f.x,.025,f.z,palette.tile);
 ...
 function compactOffice(layout){
  box(1.55,.07,.60,5.32,.74,.48,palette.wood);for(const x of [4.62,6.02])box(.04,.74,.48,x,0,.48,palette.wood);chair(5.30,1.13);
  box(.54,.34,.04,5.32,.84,.31,'#35473e');
  if(layout.config.splitFlex){sofa(5.29,1.80,0,1.78);for(let i=0;i<5;i++)box(1.33,.04,.46,5.61,.27+i*.35,3.48,palette.wood);}
 }
 function smallStudy(layout){
  const mid=(3.27+layout.divider)/2;box(layout.divider-3.60,.07,.58,mid,.74,.48,palette.wood);
  box(.45,.68,.48,3.73,.05,.48,palette.wall);box(.45,.68,.48,layout.divider-.43,.05,.48,palette.wall);chair(mid,1.1);
  sofa(mid,2.5,0,Math.min(1.65,layout.divider-3.55));
 }
 ...
 function rebuild(s){
  ...
  const outline=[[3.2,-.15],[14.24,-.15],[14.24,6.40],[8.05,6.40],[8.05,7.76],[3.75,7.76],[3.75,5.45],[-.15,5.45],[-.15,1.25],[3.2,1.25]];const shape=new THREE.Shape();outline.forEach(([x,z],i)=>i?shape.lineTo(x,-z):shape.moveTo(x,-z));shape.closePath();const geo=new THREE.ExtrudeGeometry(shape,{depth:.20,bevelEnabled:false});geo.rotateX(-Math.PI/2);const slab=mesh(geo,material('#c4c8b9'));slab.position.y=-.2;
  const floorMat=material(palette.floor,{map:floorTexture()});const layout=getLayout(s.variant);
  box(7.7,.026,1.46,7.2,0,4.60,material(palette.tile));box(2.85,.026,2.30,9.475,0,5.05,palette.tile);
  ...
  for(const w of architecture.walls)wall(w.x1,w.z1,w.x2,w.z2,{thick:w.thick,height:w.glass?2.4:w.external?2.75:undefined,color:w.glass?material('#bed2c8',{transparent:true,opacity:.23,depthWrite:false}):palette.wall});
  for(const b of architecture.structuralBlocks)box(b.w,b.h,b.d,b.x,0,b.z,palette.wall);
  for(const w of architecture.windows)windowUnit(w.x,w.z,w.width,w.rotation);
  for(const d of architecture.doors){
   const rotation=d.axis==='z'?(d.hingeEnd?Math.PI/2:-Math.PI/2):0;
   if(d.sliding){for(const [a,b] of [[d.z-.72,d.z-.02],[d.z+d.width+.02,d.z+d.width+.72]])wall(d.x-.05,a,d.x-.05,b,{thick:.025,height:2.4,color:material('#bdcfc5',{transparent:true,opacity:.27,depthWrite:false})});line([d.x,2.42,d.z],[d.x,2.42,d.z+d.width],'#687a6c');}
   else door(d.x,d.z+(d.hingeEnd?d.width:0),d.width,rotation,d.swing);
  ...
  if(s.furniture){
   bed(layout);if(layout.masterSuite)masterDressing(layout);else if(layout.config.splitFlex||layout.config.openMiddle)compactOffice(layout);else if(layout.divider<6)smallStudy(layout);else flexRoom();kitchen();bathroom(architecture);
   for(const f of architecture.hallFurniture){box(f.w,f.h,f.d,f.x,.025,f.z,palette.wood);round(f.w-.03,.06,f.d-.02,f.x,f.h+.025,f.z,palette.fabric,.05);}
 ...
 function setView(view){
  activeView=view;if(view==='shower')selectRoom('shower');
  const target=view==='shower'?new THREE.Vector3(12.1,.55,5.05):view==='kitchen'?new THREE.Vector3(10.4,.35,1.95):new THREE.Vector3(6.5,.35,3.55);
  const direction=(view==='shower'?new THREE.Vector3(-.25,1.45,.25):view==='top'?new THREE.Vector3(0,1,.001):view==='kitchen'?new THREE.Vector3(.15,.74,.65):new THREE.Vector3(.40,.68,.62)).normalize();
  const right=new THREE.Vector3().crossVectors(new THREE.Vector3(0,1,0),direction).normalize();
  const up=new THREE.Vector3().crossVectors(direction,right).normalize();
  const bounds=view==='shower'?[[10.05,0,3.75],[14.25,2.75,6.4]]:view==='kitchen'?[[6.4,0,-.2],[14.4,2.75,4.0]]:[[-1.4,0,-.2],[14.4,2.75,7.9]];
 ...
 function render(){renderer.render(scene,camera);for(const l of labels){const pos=l.pos.clone().project(camera);const visible=activeView!=='shower'&&pos.z>-1&&pos.z<1&&Math.abs(pos.x)<.96&&Math.abs(pos.y)<.96;l.el.style.display=visible?'':'none';l.el.style.left=((pos.x+1)/2*container.clientWidth)+'px';l.el.style.top=((-pos.y+1)/2*container.clientHeight)+'px';}}
 ...
 return {rebuild,setView,resize,capture(){render();const c=document.createElement('canvas');c.width=renderer.domElement.width;c.height=renderer.domElement.height;const ctx=c.getContext('2d');ctx.fillStyle='#eeeee7';ctx.fillRect(0,0,c.width,c.height);ctx.drawImage(renderer.domElement,0,0);ctx.fillStyle='#596b51';ctx.font=`${Math.max(15,c.width/65)}px Arial`;ctx.fillText('КВАРТИРА · Эскиз 04 / '+(proposals[state.variant]?.label||'Базовая схема'),c.width*.03,c.height*.95);return c.toDataURL('image/png');},destroy(){cancelAnimationFrame(raf);observer.disconnect();controls.dispose();renderer.dispose();}};
}
