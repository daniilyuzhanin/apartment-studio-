...
export function createApartment(container,onRoom){
 ...
 function kitchen(){
  for(let i=0;i<6;i++){const x=10.8+i*.6;const tall=i===5&&state.variant!=='peninsula'||i===0&&state.variant==='peninsula';const h=tall?2.3:.86;box(.584,h,.62,x,.06,3.40,palette.kitchen);box(.57,.035,.017,x,h*.67,3.077,palette.wood);if(tall)box(.57,.028,.012,x,1.38,3.075,'#525d4d');}
  ...
  if(state.variant==='peninsula'){box(.85,.87,1.72,13.4,.05,2.23,palette.kitchen);round(1.03,.07,1.86,13.4,.94,2.21,'#e4ddcb',.07);chair(12.65,1.86,Math.PI/2);chair(12.65,2.55,Math.PI/2);dining(10.77,1.02,1.35,.85,4);}
  else dining(11.65,1.45,1.9,.9,6);
 }
 function bathroom(){
 ...
 function rebuild(s){
  ...
  // Curved glazed loggia, separate from the heated rooms.
  const bshape=new THREE.Shape();bshape.moveTo(3.2,0);bshape.lineTo(-.6,0);bshape.quadraticCurveTo(-1.35,0,-1.25,-.65);bshape.lineTo(-.15,-3.5);bshape.lineTo(-.15,-1.25);bshape.lineTo(3.2,-1.25);bshape.closePath();const bg=new THREE.ExtrudeGeometry(bshape,{depth:.16,bevelEnabled:false});bg.rotateX(-Math.PI/2);const bs=mesh(bg,material('#c6cbb9'));bs.position.y=-.15;
  const railing=[[-.15,3.5],[-.6,2.1],[-1.1,.8],[-1.25,.4],[-1.1,.13],[-.7,0],[.1,0],[1.2,0],[2.2,0],[3.2,0]];for(let i=1;i<railing.length;i++){const a=railing[i-1],b=railing[i];wall(...a,...b,{height:1.3,thick:.022,color:material('#bdcec1',{transparent:true,opacity:.2})});line([a[0],1.32,a[1]],[b[0],1.32,b[1]],'#596f61');box(.04,1.35,.04,a[0],0,a[1],'#5b6c5d');}plant(2.78,.59,.60);cyl(.25,.42,1.48,.01,.62,palette.wood);chair(.73,.70,-Math.PI/2);
  ...
  segmentedWall(-.075,3.27,1.33,[[1.3,2.25,'window'],[2.25,3.0,'door']]);wall(3.27,0,3.27,3.78,{thick:.2});wall(3.27,4.70,3.27,5.38,{thick:.2});
  wall(6.425,0,6.425,3.82);segmentedWall(3.27,6.425,3.82,[[4.25,5.10]]);
  if(s.variant==='original')wall(9.57,0,9.57,3.82);
  segmentedWall(6.425,9.57,3.82,[[7.35,8.23]]);segmentedWall(9.57,14.2,3.82,[[9.62,10.44]]);
  ...
  // Door thresholds preserve openings and circulation.
  for(const [x,z,w] of [[4.68,3.82,.86],[7.8,3.82,.86],[10.04,3.82,.82],[5.14,5.38,.85],[9.25,6,.95]])box(w,.025,.16,x,.027,z,palette.wood);
  if(s.furniture){bed();flexRoom();kitchen();bathroom();if(s.variant==='original'){sofa(7.95,1.00,0,2.25);rug(7.98,2.25,2.4,2);coffee(7.95,2.12);}else{rug(7.97,2.03,2.8,2.8);sofa(7.98,.91);coffee(7.92,2.24);box(2.10,.38,.32,7.96,.10,3.45,palette.wood);box(1.30,.75,.065,7.96,.72,3.5,'#39483a');plant(8.99,3.05,.68);}box(.54,2.18,1.3,10.61,.04,5.10,palette.wall);box(1.13,.45,.42,8.88,.02,5.72,palette.wood);round(1.1,.07,.42,8.88,.47,5.72,palette.fabric,.06);for(let i=0;i<4;i++)box(1.30,.045,.42,7.16,.3+i*.45,6.7,palette.wood);plant(13.66,.43,.9);}
 ...
 return {rebuild,setView,resize,capture(){render();const c=document.createElement('canvas');c.width=renderer.domElement.width;c.height=renderer.domElement.height;const ctx=c.getContext('2d');ctx.fillStyle='#eeeee7';ctx.fillRect(0,0,c.width,c.height);ctx.drawImage(renderer.domElement,0,0);ctx.fillStyle='#596b51';ctx.font=`${Math.max(15,c.width/65)}px Arial`;ctx.fillText('КВАРТИРА · Эскиз / '+({original:'Исходная схема',table:'Большой стол',peninsula:'Полуостров'}[state.variant]),c.width*.03,c.height*.95);return c.toDataURL('image/png');},destroy(){cancelAnimationFrame(raf);observer.disconnect();controls.dispose();renderer.dispose();}};
}
