import {room,fixtures,shaft,toilet,door,niche,showerSystem,lights,power,designs} from './bathroom-data.js?v=3';
...
function planBase(){let s=rect(0,0,room.width,room.depth,'floor');
 ...
 s+=`<circle cx="${X(showerSystem.headX)}" cy="${Y(showerSystem.z)}" r="19" class="shower-head"/>`+line(X(showerSystem.headX),Y(showerSystem.z),X(showerSystem.x),Y(showerSystem.z),'fixture');
...
export function drawing(kind,design='stone'){if(kind==='elevation')return elevations(design);let s=planBase();
 if(kind==='layout'){s+=bubble(.83,.25,'01')+bubble(.425,2.03,'02')+bubble(1.36,1.84,'03')+bubble(2.42,1.1,'04')+bubble(niche.x,.25,'05');s+=text(X(2.14),Y(.25),'ШАХТА','tiny');s+=text(X(-.55),Y(1.95),'БАНКЕТКА','tiny');s+=dimY(0,.503,-.32,'503');}
...
function elevations(design){let s='';const p=designs[design]||designs.stone;const scale=.112,y0=80;const areas=[{x:45,y:y0,w:2.97,t:'А / ШКАФ, ШАХТА, ПОЛКИ',type:'cabinet'},{x:475,y:y0,w:2.28,t:'Б / ДУШ · ПРАВАЯ СТЕНА',type:'shower'},{x:45,y:465,w:2.97,t:'В / РАКОВИНА И УНИТАЗ',type:'vanity'},{x:475,y:465,w:2.28,t:'Г / ВХОД',type:'door'}];
 ...
 if(a.type==='shower'){s+=r(0,0,2280,2650,'timber');for(let i=200;i<2280;i+=200)s+=line(a.x+i*scale,a.y,a.x+i*scale,a.y+h,'tile');s+=line(a.x+1300*scale,a.y+510*scale,a.x+1300*scale,a.y+1550*scale,'fixture')+line(a.x+1188*scale,a.y+510*scale,a.x+1412*scale,a.y+510*scale,'fixture')+r(0,2580,2280,15,'light-fill');}
